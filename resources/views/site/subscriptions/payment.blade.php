...
<div class="py-10 bg-gradient-to-r from-etudes-blue/[.2] to-etudes-orange/[.1]">
    <div class="bg-white max-w-2xl mx-auto rounded pb-4">
        <div class="">
            <video src="{{asset('site/assets/subscriptions/spot.mp4')}}" class="h-80 md:h-96 mx-auto rounded-xl md:mt-2 animate__animated animate__fadeInDown" autoplay muted loop controls></video>
        </div>
        <div class=" py-4 text-center font-bold text-white italic text-2xl">
            <span class="bg-etudes-orange px-2 py-1"><del class="text-sm pr-3 font-medium">29.900 Fcfa</del>14.900 Fcfa</span>
        </div>
        ...
        @if(!Auth::check())
            ...
            {{-- <div class="mt-4 mx-2 md:mx-8">
                <div class="h-10 border rounded-lg flex items-center justify-between">
                    <input type="text" class="h-full bg-transparent w-3/4 border-r rounded-l-lg px-2 focus:outline-none focus:border focus:border-etudes-orange" placeholder="Code promotionel">
                    <button class="w-1/4 h-full hover:bg-etudes-orange hover:font-bold hover:text-white hover:uppercase hover:italic rounded-r-lg duration-300">Appliquer</button>
                </div>
            </div> --}}
            ...
            <form action="{{route('subcription.login')}}" class="hidden" method="post" id="login-form">
                @csrf
        ...
        <div class="border mt-6 p-2 mx-2 md:mx-8 rounded-lg">
            <div>
                <img src="{{asset('site/assets/subscriptions/bois.png')}}" class="h-12 mx-auto" alt="">
            </div>
            <div class="mx-2 md:mx-8 mt-4 space-y-4">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <div class="w-3/4 md:w-1/2">
                        <span class="px-2 py-1 bg-etudes-orange uppercase font-bold italic text-white text-sm" id='user-email'>{{Auth::user()->email}}</span>
                    </div>
                </div>
                <div class="flex justify-between items-center">
                    <div class="w-1/4 md:w-1/2 font-semibold">
                    ...
                    <div class="w-3/4 md:w-1/2">
                        <span class="px-2 py-1 bg-etudes-orange uppercase font-bold italic text-white text-sm" id='user-last_name'>{{Auth::user()->last_name}}</span>
                    </div>
                </div>
                <div class="flex justify-between items-center">
                    <div class="w-1/4 md:w-1/2 font-semibold">
                    ...
                    <div class="w-3/4 md:w-1/2">
                        <span class="px-2 py-1 bg-etudes-orange uppercase font-bold italic text-white text-sm" id='user-first_name'>{{Auth::user()->first_name}}</span>
                    </div>
                </div>
...
<script src="{{asset('site/assets/js/subscription.js')}}"></script>
