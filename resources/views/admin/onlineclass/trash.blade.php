@extends('admin.base', [
    'title' => 'Corbeille des Onlines Classrooms',
    'active' => 'online-class',
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Liste des online class supprimées</h4>
                <div>
                    <a href="{{route('admin.onlineClass.create')}}">
                        <button class="btn btn-primary">Ajouter une online class</button>
                    </a>
                    <a href="{{route('admin.onlineClass.index')}}">
                        <button class="btn btn-success text-white">Liste onlines class</button>
                    </a>
                    <a href="{{route('admin.onlineClass.index')}}"><button class="btn btn-secondary">Corbeille</button></a>
                </div>
            </div>
            <div class="card-body">
                            ...
                            @foreach ($onlineClasses as $onlineClass)
                                <tr>
                                    ...
                                    <td class="text-truncate text-center">
                                        <a href="{{$onlineClass->restoreUrl($onlineClass->slug)}}">
                                            <button data-toggle="modal" class="btn btn-success text-white">Restaurer</button>
                                        </a>
                                        <button data-toggle="modal" class="btn btn-danger text-white" data-target="#deleteModal{{$onlineClass->id}}">Supprimer</button>
                                        <!-- DELETE Modal -->
                                        <div class="modal fade" id="deleteModal{{$onlineClass->id}}" >
                                            <div class="modal-dialog" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{route('admin.onlineClass.forceDelete')}}" method="post">
                                                        @csrf
